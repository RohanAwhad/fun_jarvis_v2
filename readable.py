...
class Readable:
    def __init__(self, url):
        ...
        if self.response is not None:
            ...
            self.article = str(self._grab_article_content())
            self.text = BeautifulSoup(self.article, 'lxml').text.strip()
    ...
    def _get_response(self):
        ...
        if res.status_code != 200:
            logger.warning(f'Failed to get url: {url}. Received: {res.status_code} code and "{res.text}" msg')
            return None
        return res
    ...
    def _remove_unlikely_candidate(self, node):
        ...
        if regexps['unlikelyCandidatesRe'].search(unlikely_match_string) is not None and regexps['okMaybeItsACandidateRe'].search(unlikely_match_string) is None and node.name != 'html' and node.name != 'body':
            logger.debug('Removing unlikely candidate - ' + unlikely_match_string)
            node.decompose()
            return True
    ...
    def _convert_textnode_followed_by_br_to_para_node(self, node):
        logger.debug('Altering textnode followed by br to para node')
        new_node = self.soup.new_tag('p')
        new_node.string = node.string
    ...
    def _convert_span_with_text_to_para(self, node):
        logger.debug('Replacing text node with a span tag with the same content.')
